-- Ensure audit logs exist in the live database and force PostgREST to reload
-- its schema cache. This repairs projects where earlier audit migrations did
-- not apply or the REST API still cannot see public.audit_logs.
...
CREATE INDEX IF NOT EXISTS audit_logs_record_action_created_idx ON public.audit_logs (record_id, action, created_at DESC);
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    IF jsonb_typeof(new_custom) = 'object' THEN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    new_custom := COALESCE(NEW.custom_fields, '{}'::jsonb);
    IF jsonb_typeof(old_custom) = 'object' AND jsonb_typeof(new_custom) = 'object' THEN
      FOR custom_key IN
  ...
  old_custom := COALESCE(OLD.custom_fields, '{}'::jsonb);
  IF jsonb_typeof(old_custom) = 'object' THEN
